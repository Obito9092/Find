...
local function updatePlacePrompts(userId)
	...
	if equippedTool and equippedTool:FindFirstChild("Rarity") then
	...
	if backpack and not hasBrainrotTool then
		for _, item in pairs(backpack:GetChildren()) do
			if item:IsA("Tool") and item:FindFirstChild("Rarity") then
				hasBrainrotTool = true
				break
			end
		end
...
local function giveEggToPlayer(player, eggModel, eggName, eggData)
	...
	if clonedEgg:IsA("Model") then
		for _, obj in pairs(clonedEgg:GetDescendants()) do
			...
			if obj:IsA("ProximityPrompt") then obj:Destroy() end
			if obj:IsA("Weld") or obj:IsA("WeldConstraint") or obj:IsA("Motor6D") then obj:Destroy() end
		end
		...
		if not clonedEgg.PrimaryPart then
			...
			if not clonedEgg.PrimaryPart then
				...
				if largestPart then clonedEgg.PrimaryPart = largestPart end
			end
		end
	else
...
local function breakEggInSwap(egg, player, eggData)
	...
	for i = 1, 12 do
		...
		fragment.Velocity = Vector3.new(math.random(-10, 10), math.random(5, 15), math.random(-10, 10))
		fragment.RotVelocity = Vector3.new(math.random(-20, 20), math.random(-20, 20), math.random(-20, 20))
	...
	TweenService:Create(displayPart, TweenInfo.new(3, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut, -1, true), {
		Position = safePosition + Vector3.new(0, 2, 0)
	}):Play()

	-- auto cleanup after 30s if nobody picks it up
	task.spawn(function()
		task.wait(30)
...
local function playCollectionAnimation(placedItem)
	...
	if placedItem:IsA("Model") then
		...
		local scaleTween = TweenService:Create(targetPart, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Size = originalSize * 1.2})
		scaleTween:Play()
		scaleTween.Completed:Wait()
		TweenService:Create(targetPart, TweenInfo.new(0.2, Enum.EasingStyle.Elastic, Enum.EasingDirection.Out), {Size = originalSize}):Play()
	else
		local originalSize = targetPart.Size
		local originalCFrame = targetPart.CFrame

		local jumpTween = TweenService:Create(targetPart, TweenInfo.new(0.15, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {CFrame = originalCFrame * CFrame.new(0, 2, 0)})
		jumpTween:Play()
		jumpTween.Completed:Wait()
		TweenService:Create(targetPart, TweenInfo.new(0.15, Enum.EasingStyle.Bounce, Enum.EasingDirection.In), {CFrame = originalCFrame}):Play()
	end
end
...
local function setupTouchCollect(placedItem, userId)
	...
	local function setupPartTouch(part)
		...
		part.Touched:Connect(function(hit)
			...
			local currentTime = tick()
			if touchCooldowns[player.UserId] and currentTime - touchCooldowns[player.UserId] < 1 then return end

			local accumulated = placedItem:FindFirstChild("AccumulatedMoney")
			if not accumulated or not accumulated:IsA("NumberValue") then return end
...
local function loadPlacedBrainrot(placePart, placeNumber, brainrotData, userId, rebirths)
	...
	if not brainrotTemplate then
		warn("[load] couldnt find template:", brainrotData.ItemName)
		return
	end
	...
	print("[load] loaded:", brainrotData.ItemName, "at place", placeNumber)
end
				...
				if carriedEgg then
					...
					if placedEgg:IsA("Model") then
						for _, obj in pairs(placedEgg:GetDescendants()) do
							...
							if obj:IsA("ProximityPrompt") then obj:Destroy() end
							if obj:IsA("Weld") or obj:IsA("Motor6D") then obj:Destroy() end
						end
						...
						if not placedEgg.PrimaryPart then
							...
							if not placedEgg.PrimaryPart then
								...
								if largestPart then placedEgg.PrimaryPart = largestPart end
							end
						end
						...
						local eggSize = placedEgg.PrimaryPart and placedEgg.PrimaryPart.Size.Y or 0
					...
					if player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
						...
						hrp.CFrame = CFrame.new(spawnPart.Position + Vector3.new(0, 3, -8))
					end
				end
			end)
		end
...
local function setupPlacePart(placePart, userId)
	...
	placePrompt.Triggered:Connect(function(player)
		...
		print("[place]", itemName, "at place", placeNumber, "(x"..multiplier.." = "..finalMoney.."/s)")
	end)
end

local function findEggDataByPart(part)
	if not part then return nil, nil end
	if eggs[part] then return part, eggs[part] end
	return nil, nil
end
...
local function initializePlot(fullPlot, userId)
	local swapPart = fullPlot:FindFirstChild("Swip", true) or fullPlot:FindFirstChild("Swap", true)
	if swapPart then
...
LoadBrainrotsEvent.Event:Connect(function(fullPlotName, userId, placedBrainrotsData, rebirths)
	...
	print("[load] all brainrots loaded for", fullPlotName)
end)
...
Players.PlayerAdded:Connect(function(player)
	...
	player.CharacterAdded:Connect(function(character)
		character.ChildAdded:Connect(function(child)
			if child:IsA("Tool") and child:FindFirstChild("Rarity") then
